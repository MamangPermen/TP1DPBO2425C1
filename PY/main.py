...
def tambah_barang(daftar):
    ...
    daftar.append(Barang(id, nama, harga, stok))
...
def hapus_barang(daftar):
    ...
    for i, barang in enumerate(daftar):
...
def update_barang(daftar):
    ...
    for barang in daftar:
        if barang.cocok_id(id):
            ...
            barang.edit_data(nama_baru, harga_baru, stok_baru)
...
def tampilkan_semua_barang(daftar):
    if not daftar:
        print("Data kosong\n")
    else:
...
def main():
    ...
    while menu != 6:
        tampilkan_menu()
        try:
            menu = int(input("Masukkan perintah : ").strip())
        except ValueError:
